...
public final class SearchRequestBuilder {
  ...
  public static String toJson(Query query) {
    ...
    JsonGenerator generator = new JacksonJsonpMapper().jsonProvider().createGenerator(writer);
    query.serialize(generator, new JacksonJsonpMapper());
    generator.flush();
    return writer.toString();
  }

  public static org.opensearch.client.opensearch.core.SearchRequest buildRequest(
      String indexName, org.opensearch.client.opensearch._types.query_dsl.Query query) {
  ...
  public static org.opensearch.client.opensearch.core.SearchRequest buildRequest(
      String indexName, SearchRequest request) {
      ...
      builder
          .query(
              q ->
                  q.multiMatch(
                      m ->
                          m.fields(request.highlight().fields())
  ...
  public static org.opensearch.client.opensearch.core.SearchRequest buildFuzzyRequest(
      String indexName, SearchRequest request) {
    ...
    for (Range<Float> range : ranges) {
      AggregationRange aggregationRange =
          new AggregationRange.Builder()
              ...
              .to(JsonData.of(range.to()))
              .build();
      aggregationRanges.add(aggregationRange);
    }
    return new Aggregation.Builder()
        .range(r -> r.field(rangeFacet.field()).ranges(aggregationRanges))
        .build();
  }

  private static Aggregation buildHistogramAggregation(HistogramFacet intervalFacet) {
    return new Aggregation.Builder()
        .histogram(r -> r.field(intervalFacet.field()).interval(intervalFacet.interval()))
        .build();
  }
  ...
  private static @NonNull Aggregation buildDateRangeAggregation(DateRangeFacet rangeFacet) {
    ...
    for (Range<String> range : ranges) {
      DateRangeExpression aggregationRange =
    ...
    return new Aggregation.Builder()
        .dateRange(r -> r.field(rangeFacet.field()).ranges(aggregationRanges).format("yyyy-MM-dd"))
        .build();
  }
  ...
  private static Highlight buildHighlights(SearchRequest request) {
    ...
    for (String field : fields) {
      highlightFields.put(
          field,
          HighlightField.of(hf -> hf.numberOfFragments(0))); // 0 fragments returns the entire field
    }
        ...
        h ->
            h.type(
                    HighlighterType.of(
                        ht ->
                            ht.builtin(
                                BuiltinHighlighterType.Unified))) // Use the unified highlighter
