...
public class SearchClientImpl implements SearchClient {
  ...
  public <T extends Document> OperationResult saveOrUpdate(
      String indexName, @Nonnull Collection<T> documents) {
    List<BulkOperation> indexOperations = new ArrayList<>(documents.size());
    documents.forEach(
        d ->
            indexOperations.add(
                new BulkOperation.Builder()
                    .index(IndexOperation.of(i -> i.index(indexName).id(d.docId()).document(d)))
                    .build()));
  ...
  private OperationResult deleteByQuery(String indexName, Query query) {
    DeleteByQueryRequest deleteRequest =
        DeleteByQueryRequest.of(d -> d.index(indexName).query(query).waitForCompletion(false));
  ...
  public long countByQuery(String indexName, SearchRequest request) {
    ...
    CountRequest countRequest =
        new CountRequest.Builder().index(indexName).query(boolQuery).build();
  ...
  public <T> List<T> findAll(String indexName, Class<T> clazz) {
    ...
    org.opensearch.client.opensearch.core.SearchRequest searchRequest =
        SearchRequestBuilder.buildRequest(indexName, matchAllQuery);
  ...
  public SearchResult search(String indexName, SearchRequest request) {
    org.opensearch.client.opensearch.core.SearchRequest searchRequest =
        SearchRequestBuilder.buildRequest(indexName, request);
