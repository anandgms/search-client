...
import edu.anand.search.oss.util.QueryBuilder;
...
public class SearchClientImpl implements SearchClient {
    ...
    public OperationResult deleteByQuery(String indexName, SearchRequest request) {
        Query boolQuery = QueryBuilder.buildQuery(request);
    ...
    public <T> List<T> findAll(String indexName, Class<T> clazz) {
        Query matchAllQuery = Query.of(q -> q.matchAll(m -> m));
        return findByQuery(indexName, matchAllQuery, clazz);
    }

    @Override
    public SearchResult search(String indexName, SearchRequest request) {
        Query boolQuery = QueryBuilder.buildQuery(request);
        List<NamedField> documents = findByQuery(indexName, boolQuery, NamedField.class);
        return new SearchResult().setDocuments(documents);
    }

    // Find all documents that match the query
    private <T> List<T> findByQuery(String indexName, Query query, Class<T> clazz) {
    ...
    @Override
    public SearchResult fuzzySearch(String indexName, FuzzyRequest request) {
        Query fuzzyQuery = QueryBuilder.buildQuery(request);
        org.opensearch.client.opensearch.core.SearchRequest searchRequest =
                new org.opensearch.client.opensearch.core.SearchRequest.Builder()
                        .index(indexName).query(fuzzyQuery).build();

        List<NamedField> suggestions = findByQuery(indexName, fuzzyQuery, NamedField.class);
        return new SearchResult().setDocuments(suggestions);
    }
}
