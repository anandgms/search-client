...
public interface SearchClient {
  ...
  /**
   * Returns true if there is one or more documents that match the search criteria in the specified
   * index.
   */
  ...
  /**
   * Finds all documents from the specified index. Suitable for small indices, as this method does
   * not support pagination.
   */
  <T> List<T> findAll(String indexName, Class<T> clazz);

  /**
   * Finds all documents that match the search criteria from the specified index. Pagination is
   * controlled using the page and size in SearchRequest.
   *
   * <p>For suggestions (case-insensitive n-gram search), use fuzzyQuery instead of query. Note that
   * filters, facets, highlighting, pagination are ignored in fuzzy search.
   */
